...
**Reviewed:** {YYYY-MM-DD}
**Specialist:** {Security | Engineering | Ops | Product | AI}
...
*Full specialist findings: [security-findings.md](security-findings.md) | [engineering-findings.md](engineering-findings.md) | ...*
```

### Formatting Rules (MANDATORY)

1. **Verdict first.** Lead with the bottom line.
2. **Top 3 Risks use blockquotes** for cross-specialist agreement signals.
3. **Tables are compact.** One line per cell.
4. **Per-specialist sections are bullet-only.** Bold headline + one sentence.
5. **Horizontal rules (`---`) between major sections.**
6. **Recommended Actions is a table**, not a numbered prose list.
7. **Open Questions are one line each.**
